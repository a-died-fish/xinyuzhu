...
from feature_matching.utils import get_client_centroids_info, get_global_centroids, personalized_get_client_centroids_info
...
def get_args():
    ...
    parser.add_argument('--datadir', type=str, required=False, default="/GPFS/data/ruiye/fssl/dataset", help="Data directory")
...
def train_net(net_id, net, train_dataloader, generalized_test_dataloader, personalized_test_dataloader, epochs, lr, args_optimizer, args, device="cpu"):
...
def train_net_fm(net_id, net, train_dataloader, generalized_test_dataloader, personalized_test_dataloader, epochs, lr, args_optimizer, args, global_centroids, start_fm, device="cpu"):
    # net = nn.DataParallel(net)
    net.cuda()
    logger.info('Training network %s' % str(net_id))
    logger.info('n_training: %d' % len(train_dataloader))
    logger.info('n_test: %d' % len(generalized_test_dataloader))
    ...
    # train_acc, _ = compute_accuracy(net, train_dataloader, device=device)
    generalized_test_acc, _ = compute_accuracy(net, generalized_test_dataloader, get_confusion_matrix=False, device=device)
    personalized_test_acc, _ = compute_accuracy(net, personalized_test_dataloader, get_confusion_matrix=False, device=device)

    logger.info('>> Generalized Test accuracy: %f' % generalized_test_acc)
    logger.info('>> Personalized Test accuracy: %f' % personalized_test_acc)

    net.to('cpu')

    logger.info(' ** Training complete **')
    return generalized_test_acc, personalized_test_acc

def train_net_fedprox(net_id, net, global_net, train_dataloader, generalized_test_dataloader, personalized_test_dataloader, epochs, lr, args_optimizer, mu, args,
                      device="cpu"):
...
def train_net_fedcon(net_id, net, global_net, previous_nets, train_dataloader, generalized_test_dataloader, personalized_test_dataloader, epochs, lr, args_optimizer, mu, temperature, args,
                      round, device="cpu"):
    # net = nn.DataParallel(net)
    net.cuda()
...
def local_train_net(nets, args, net_dataidx_map, train_dl=None, generalized_test_dl=None, personalized_test_dls=None, global_model = None, prev_model_pool = None, server_c = None, clients_c = None, round=None, device="cpu", current_round=0):
    generalized_avg_acc = 0.0
    generalized_acc_list = []
    personalized_avg_acc = 0.0
    personalized_acc_list = []
    if global_model:
        global_model.cuda()
    if server_c:
        server_c.cuda()
    ...
    for net_id, net in nets.items():
        ...
        personalized_test_dl=personalized_test_dls[net_id]
        # train_dl_global, test_dl_global, _, _ = get_dataloader(args.dataset, args.datadir, args.batch_size, 32)
        n_epoch = args.epochs
        if args.alg == 'fedavg' or args.alg == 'fednova':
            testacc = train_net(net_id, net, train_dl_local, generalized_test_dl, personalized_test_dl, n_epoch, args.lr, args.optimizer, args,
                                        device=device)
        elif args.alg =='fedfm' or args.alg =='perfedfm' or args.alg == 'fedproto':
            generalized_test_acc, personalized_test_acc = train_net_fm(net_id, net, train_dl_local, generalized_test_dl, personalized_test_dl, n_epoch, args.lr, args.optimizer, args, global_centroids, current_round>=args.start_ep_fm,
                                        device=device)
        elif args.alg == 'fedprox':
            testacc = train_net_fedprox(net_id, net, global_model, train_dl_local, generalized_test_dl, personalized_test_dl, n_epoch, args.lr,
                                                  args.optimizer, args.mu, args, device=device)
        elif args.alg == 'moon':
            prev_models=[]
            for i in range(len(prev_model_pool)):
                prev_models.append(prev_model_pool[i][net_id])
            trainacc, testacc = train_net_fedcon(net_id, net, global_model, prev_models, train_dl_local, generalized_test_dl, personalized_test_dl, n_epoch, args.lr,
                                                  args.optimizer, args.mu, args.temperature, args, round, device=device)
        elif args.alg == 'local_training':
            trainacc, testacc = train_net(net_id, net, train_dl_local, generalized_test_dl, personalized_test_dl, n_epoch, args.lr, args.optimizer, args,
                                          device=device)
        # logger.info("net %d final test acc %f" % (net_id, testacc))
        generalized_avg_acc += generalized_test_acc
        generalized_acc_list.append(generalized_test_acc)
        personalized_avg_acc += personalized_test_acc
        personalized_acc_list.append(personalized_test_acc)
    generalized_avg_acc /= args.n_parties
    personalized_avg_acc /= args.n_parties
    print(f'generalized_avg_acc : {generalized_avg_acc}')
    print(f'personalized_avg_acc : {personalized_avg_acc}')
    logger.info("END: Averaged Generalized Test ACC %f" % generalized_avg_acc)
    logger.info("END: Averaged Personalized Test ACC %f" % personalized_avg_acc)
    if args.alg == 'local_training':
        logger.info("generalized_avg test acc %f" % generalized_avg_acc)
        logger.info("generalized_std acc %f" % np.std(generalized_acc_list))
    if global_model:
        global_model.to('cpu')
    if server_c:
        ...
        server_c.to('cpu')
    return nets, generalized_avg_acc, personalized_avg_acc
...
if __name__ == '__main__':
    ...
    net_dataidx_map_test = partition_test_data(traindata_cls_counts=traindata_cls_counts, y_test=y_test, n_parties=args.n_parties)
    ...
    if args.server_momentum:
        ...
        for key in moment_v:
            moment_v[key] = 0
    print(device)
    if args.alg=='fedfm' or args.alg=='perfedfm' or args.alg=='fedproto':
    ...
    test_local_dls=[]
    for net_id, net in nets.items():
        train_dataidxs = net_dataidx_map[net_id]
        test_dataidxs = net_dataidx_map_test[net_id]
        train_dl_local, _, _, _ = get_dataloader(args.dataset, args.datadir, args.batch_size, 32, train_dataidxs)
        train_local_dls.append(train_dl_local)
        test_dl_local, _, _, _ = get_dataloader(args.dataset, args.datadir, args.batch_size, 32, test_dataidxs, drop_last=False)
        test_local_dls.append(test_dl_local)
    best_personalized_test_acc=0
    best_generalized_test_acc=0
    ...
    if args.alg == 'moon':
        ...
        for round in range(n_comm_rounds):
            ...
            val_acc, _ = compute_accuracy(global_model, test_local_dls, device=device, multiloader=True)
        ...
        for round in range(n_comm_rounds):
            ...
            local_train_net(nets_this_round, args, net_dataidx_map, train_dl=train_local_dls, generalized_test_dl=test_dl, personalized_test_dls=test_local_dls, device=device)
        ...
        for round in range(n_comm_rounds):
            ...
            local_train_net(nets_this_round, args, net_dataidx_map, train_dl=train_local_dls, generalized_test_dl=test_dl, personalized_test_dls=test_local_dls, device=device, current_round=round)
        ...
        for round in range(n_comm_rounds):
            ...
            _, generalized_avg_acc, personalized_avg_acc = local_train_net(nets_this_round, args, net_dataidx_map, train_dl=train_local_dls, generalized_test_dl=test_dl, personalized_test_dls=test_local_dls, device=device, current_round=round)
            if personalized_avg_acc>best_personalized_test_acc:
                best_personalized_test_acc = personalized_avg_acc
            if generalized_avg_acc>best_generalized_test_acc:
                best_generalized_test_acc = generalized_avg_acc
            ...
            mkdirs(args.modeldir+'perfedfm/')
            global_model.to('cpu')

            torch.save(global_model.state_dict(), args.modeldir+'perfedfm/'+'globalmodel'+args.log_file_name+'.pth')
            torch.save(nets[0].state_dict(), args.modeldir+'perfedfm/'+'localmodel0'+args.log_file_name+'.pth')
        ...
        for round in range(n_comm_rounds):
            ...
            #feature_matching
            global_centroids = global_centroids.cpu()
            local_centroids, local_distributions = personalized_get_client_centroids_info(nets_this_round, dataloaders=train_local_dls, model_name=args.model, dataset_name=args.dataset, party_list_this_round=party_list_this_round)
            global_centroids = get_global_centroids(local_centroids, local_distributions, global_centroids, momentum=0.0)
            global_centroids = global_centroids.cuda()
    ...
    print('>> Personalized Avgeage Best accuracy: %f' % best_personalized_test_acc)
    print('>> Generalized Avgeage Best accuracy: %f' % best_generalized_test_acc)
    print(args)
    print(now_time)