...
from feature_matching.loss_f import matching_cross_entropy_regression, matching_l2_regression
...
def get_args():
    ...
    parser.add_argument('--num_anchor', type=int, default=10, help='the number of anchors')
...
def train_net(net_id, net, train_dataloader, test_dataloader, epochs, lr, args_optimizer, args, device="cpu"):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x, target) in enumerate(train_dataloader):
            ...
            target = (target/100).to(torch.float32).unsqueeze(-1)
    ...
    # train_acc, _ = compute_accuracy(net, train_dataloader, device=device)
    # test_acc, conf_matrix, _ = compute_accuracy(net, test_dataloader, get_confusion_matrix=True, device=device)
    test_loss = compute_loss_regression(net, test_dataloader, get_confusion_matrix=True, device=device)
...
def local_train_net(nets, args, net_dataidx_map, train_dl=None, test_dl=None, global_model = None, prev_model_pool = None, server_c = None, clients_c = None, round=None, device="cpu", current_round=0):
    ...
    for net_id, net in nets.items():
        ...
        if args.alg == 'fedavg' or args.alg == 'fednova':
            testloss = train_net(net_id, net, train_dl_local, test_dl, n_epoch, args.lr, args.optimizer, args,
                                        device=device)
        elif args.alg =='fedfm':
            testloss = train_net_fm(net_id, net, train_dl_local, test_dl, n_epoch, args.lr, args.optimizer, args, global_centroids, current_round>=args.start_ep_fm,
                                        device=device)
            ...
            trainacc, testacc = train_net(net_id, net, train_dl_local, test_dl, n_epoch, args.lr, args.optimizer, args,
                                          device=device)
        logger.info("net %d final test loss %f" % (net_id, testloss))
        avg_loss += testloss
        loss_list.append(testloss)
    avg_loss /= args.n_parties
    if args.alg == 'local_training':
        logger.info("avg test loss %f" % avg_loss)
        logger.info("std loss %f" % np.std(loss_list))
    if global_model:
        global_model.to('cpu')
    if server_c:
...
if __name__ == '__main__':
    ...
    if args.alg=='fedfm':
        ...
        if args.dataset=='wiki':
            if args.model=='resnet18' or args.model=='resnet18_7':
                global_centroids = torch.zeros((args.num_anchor, 512))
            elif args.model=='resnet50' or args.model=='resnet50_7':
                global_centroids = torch.zeros((args.num_anchor, 2048))
    ...
    if args.alg == 'moon':
        ...
        for round in range(n_comm_rounds):
            ...
            # val_acc, _ = compute_accuracy(global_model, val_local_dls, device=device, multiloader=True)
            # test_acc, conf_matrix, _ = compute_accuracy(global_model, test_dl, get_confusion_matrix=True, device=device)
            val_loss = compute_loss_regression(global_model, val_local_dls, device=device, multiloader=True)
            test_loss = compute_loss_regression(global_model, test_dl, get_confusion_matrix=True, device=device)

            if(best_loss>val_loss):
                best_loss=val_loss
                best_test_loss=test_loss
                logger.info('New Best val loss:%f , test loss:%f'%(val_loss,test_loss))
            else:
                # logger.info('>> Global Model Train accuracy: %f' % train_acc)
                logger.info('>> Global Model Train Loss: %f' % val_loss)
                logger.info('>> Global Model Test Loss: %f' % test_loss)
                logger.info('>> Global Model Best Loss: %f' % best_test_loss)
        ...
        for round in range(n_comm_rounds):
            ...
            local_centroids, local_distributions = get_client_centroids_info(global_model, dataloaders=train_local_dls, model_name=args.model, dataset_name=args.dataset, party_list_this_round=party_list_this_round, num_anchor=args.num_anchor)
            ...
            if(best_loss>val_loss):
                ...
                # logger.info('>> Global Model Train accuracy: %f' % train_loss)
                logger.info('>> Global Model Train Loss: %f' % val_loss)
                logger.info('>> Global Model Test Loss: %f' % test_loss)
                logger.info('>> Global Model Best Loss: %f' % best_test_loss)
    ...
    print('>> Global Model Best accuracy: %f' % best_test_loss)
